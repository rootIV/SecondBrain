...
- O sistema de coordenadas e compativel com o Rafinha: `TPosition`/`Position` usa dois `Single` (`float`) sem escala ou conversao. No `0x301`, o servidor preserva no broadcast o byte `Speed` recebido do client, como o Rafinha, em vez de substitui-lo por `character.SpeedMove`; posicao invalida e `MoveType` diferente de movimento normal sao rejeitados antes de atualizar o estado.
